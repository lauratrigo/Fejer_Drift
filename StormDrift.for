...
CC  Empirical vertical drifts model for storm time;
CC  After Fejer and Scherliess, JGR, 102, 24047-24056,1997;
CC  The equatorial vertical drifts are described in;
CC  SCHERLIESS AND FEJER, JGR, 104, 6829-6842, 1999;
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       PROGRAM MAIN
       ...
       CHARACTER*5 c5
       CHARACTER*20 FLname
       INTEGER FLAG,jF,jL,yymmdd(1:MAX),YMD(1:MAX)
...
CC
       WRITE(*,*) 'Input AE data file name:'
       READ(*,*) FLname
CC
c      open(20,file=FLname,mode='read')
       open(20,file=FLname)
       READ(20,*,END=30)
       DO I=1,MAX
...
30     CLOSE(20)
CC
       IF ((UT(jL)-UT(jL-1)).GT.0.5D0.AND.(UT(2)-UT(1)).GT.0.5D0) THEN
          FLAG=1   !1h resolution;
          jF=49
       ELSE
       ...
       ENDIF
C Read F107;
       WRITE(*,*) 'Input F107 data file name:'
       READ(*,*) FLname
c      open(30,file=FLname,mode='read')
       open(30,file=FLname)
       DO I=1,MAX
C2000-07-20 00:00 235.0   ""   ""
...
CC
       ...
       write(*,100) 'SLT','Drift (m/s)','Geogr. Long.','DoY','F10.7cm'
CC
c      OPEN(10,file='drift.dat',MODE='WRITE')
...
45        IF (yymmdd(iP).EQ.YMD(ij)) THEN
          ...
          END IF
C Vertical drifts during quiet time;
50        CALL vdrift_model(time,long,param,Vd_mean)
C Vertical drifts during storm times;
          CALL StormVd(FLAG,iP,AE,time,PromptVd,DynamoVd,Vd_storm)
          VdTot=Vd_mean+Vd_storm
          write(10,200) yymmdd(iP),UT(iP),Vd_mean,Vd_storm,VdTot,
     &    PromptVd,DynamoVd,AE(iP)
          write(*, 300) yymmdd(iP),UT(iP),Vd_mean,param,Vd_storm,VdTot
       END DO
       CLOSE(10)
100    format(5x,a3,6x,a11,2x,a11,6x,a3,7x,a7)
200    format(1X,I9,F8.3,7F10.3)
...
CC
CC
       SUBROUTINE MODA(IN,YEAR,MONTH,IDAY,IDOY)
C-------------------------------------------------------------------
...
C-------------------------------------------------------------------
       ...
       DIMENSION     MO(12),MO28(1:12),MO29(1:12)
       DATA MO28/0,31,59,90,120,151,181,212,243,273,304,334/
       DATA MO29/0,31,60,91,121,152,182,213,244,274,305,335/
	IMO=0
	MOBE=0
